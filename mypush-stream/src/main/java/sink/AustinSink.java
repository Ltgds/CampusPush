...
import com.ltgds.mypush.common.domain.SimpleAnchorInfo;
import io.lettuce.core.RedisFuture;
import lombok.extern.slf4j.Slf4j;
import me.chanjar.weixin.common.util.DataUtils;
import org.apache.flink.streaming.api.functions.sink.SinkFunction;
import utils.LettuceRedisUtils;
    ...
    private void realTimeData(AnchorInfo info) {
        try {
            ...
            LettuceRedisUtils.pipeline(redisAsyncCommands -> {
                 ...
                 */
                String redisMessageKey = StrUtil.join(StrUtil.COLON, PushConstant.CACHE_KEY_PREFIX,
                        PushConstant.MESSAGE_ID, info.getMessageId());
                ...
                /**
                 * 1.构建userId维度的链路信息 数据结构 list:{key, list}
                 *
                 * key:businessId
                 * listValue:[{timestamp, state, businessId}, {timestamp,state,businessId}]
                 */
                ...
                /**
                 * 2.构建消息模板维度的链路信  数据结构 hash:{key, hash}
                 *
                 * key:businessId
                ...
                redisFutures.add(redisAsyncCommands.expire(String.valueOf(info.getBusinessId()).getBytes(),
                        ((DateUtil.offsetDay(new Date(), 30).getTime()) / 1000) - DateUtil.current()));
